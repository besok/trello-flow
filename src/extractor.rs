...
impl Extractor {
    pub fn match_board_settings(&self) -> BoardSettingsManager {
        let boards: HashMap<String, BoardSettings> = self
            .trello
            .boards()
            .iter()
            .map(|b| {
                ...
                (
                    ...
                    self.dict
                        ...
                        .map(|d| BoardSettings {
                            new_card_list: lists
                                .iter()
                                .find(|l| l.name == d.list_create)
                                .map(|l| l.id.to_string())
                                .expect("list for create should be"),
                            upd_card_list: lists
                                .iter()
                                .find(|l| l.name == d.list_move)
                                .map(|l| l.id.to_string())
                                .expect("list for update should be"),
                        }),
                )
            })
    ...
    pub fn match_boards(&self) -> Vec<WordTarget> {
        ...
        let boards: HashMap<&str, &str> = boards
            .iter()
    ...
    fn col_by_boards(&self) -> HashMap<&str, &str> {
        self.dict
            ...
            .dicts
            .iter()
            .map(|d| (d.name.as_str(), d.board.as_str()))
            .collect()
    }
...
impl Extractor {
    pub fn new_from(cfg: &str, cred: &str, data: &str) -> Self {
        let trello = TrelloConnector::from_file(cred);
        let dict = DictManager::new(cfg, data);
        Extractor { trello, dict }
    }
    pub fn new(trello: TrelloConnector, dict: DictManager) -> Self {
        Extractor { trello, dict }
    }
    fn clean_up_cards(&self, board_name: &str, start_symbol: char, end_symbol: char) -> Vec<Card> {
        ...
        for c in cards.into_iter() {
            ...
            if name.contains(start_symbol) || name.contains(end_symbol) {
                let start = name
                    .chars()
                    .position(|c| c == start_symbol)
                    .expect(format!("{}", start_symbol).as_str());
                let end = name
                    .chars()
                    .position(|c| c == end_symbol)
                    .expect(format!("{}", end_symbol).as_str());
                let new_name: String = name
                    .chars()
                    .take(start)
                    .chain(name.chars().skip(end + 1))
                    .collect();
                let new_name = new_name.trim();
                let upd_card = self.trello.update_card_name(c.id.as_str(), new_name);
                println!(
                    "find a card with a wrong name:{} -> {}",
                    name, upd_card.name
                );
...
mod tests {
    ...
    fn match_boards_test() {
        ...
        let r = Extractor::new(
            TrelloConnector::from_file(cred),
            DictManager::new(cfg, data),
        );
